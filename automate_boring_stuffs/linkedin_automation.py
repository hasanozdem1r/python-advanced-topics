...
class Linkedin(object):
    ...
    def unfollow_companies(self):
        ...
        companies = self.browser.find_elements_by_css_selector('button[class*="follow is-following  follows-recommendation-card__follow-btn artdeco-button artdeco-button--tertiary artdeco-button--1 artdeco-button--fluid"]')
        # unfollow all companies
        for company in companies:
            webdriver.ActionChains(self.browser).move_to_element(company).click(company).perform()
            sleep(1)

obj_linkedin=Linkedin('your_email','your_password')

#obj_linkedin.unfollow_companies()