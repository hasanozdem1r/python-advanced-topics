...
class Email(EmailHelper):
    ...
    def send_email(self, mail_from: str, sender_name: str, mail_to: str, receiver_name: str = "God knows",
                   mail_subject: str = "Unkown") -> bool:
        """
        ...
        if not self.is_email_valid(mail_to) and not self.check_internet_connection() and not self.is_email_valid(
                mail_from):
            return False
        else:
...
if __name__ == '__main__':
    ...
    email_obj.send_email(mail_from="sender_mail", sender_name="sender_name", mail_to="receiver_email",
