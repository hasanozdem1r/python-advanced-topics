...
class ScrapeRealEstate:
    ...
    def scrape_1_page(self, country: str, id: str) -> list:
        ...
        estates = soup_obj.find("ul", {
            "class": "search-list"
        }).find_all("li", limit=100)
        estates_list = []
        estate_id = 1
    ...
    def scrape_estate_bedroom(self, estate):
        estate_bedroom = (estate.find("div", {
            "class": "specs"
        }).find("span", {
            "aria-label": "Bedrooms"
        }).text)
    ...
    def scrape_estate_bathroom(self, estate):
        estate_bathroom = (estate.find("div", {
            "class": "specs"
        }).find("span", {
    ...
    def scrape_estate_cost(self, estate):
        estate_cost = str(
            estate.find("div", {
                "class": "price serif-base"
            }).text)
        estate_cost = re.findall("[0-9,]*", estate_cost)
        estate_cost = int([item for item in estate_cost if item != ""
                          ][0].replace(",", ""))
        return estate_cost

    def scrape_estate_title(self, estate):
        estate_title = (estate.find("div", {
            "class": "details_title"
        }).find("a", {
            "class": "js_clickable"
        }).text)
